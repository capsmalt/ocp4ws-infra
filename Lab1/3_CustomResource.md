...
```
$ oc logs prometheus-monitoring-0 -c rules-configmap-reloader -n jmx-monitor
...
$ oc describe pod/prometheus-monitoring-0 -n jmx-monitor
...
```
#### コマンドレベルで変更する方/VIEDITORが不得意な方はコチラ
```
$ oc get statefulset/prometheus-monitoring -n jmx-monitor -o=jsonpath='{.spec.template.spec.containers[2].args.resources.limits}' 
map[cpu:25m memory:10Mi]
$ oc patch statefulset/prometheus-monitoring -n jmx-monitor -p 
### prometheus-config-reloaderのMemory Limitを10Miから30Miに引き上げる
```
